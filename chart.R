...
ggplot(data, aes(x = unemployment, y = baseline)) +
  geom_point(data = data %>% filter(year < 2015), color = neutral, alpha = .2, shape = 16) +
  geom_point(data = data %>% filter(year >= 2015), color = dark, shape = 16) +
  ...
  geom_text_repel(data = data %>% filter(year < 2015) %>% filter(year %in% seq(1970,2015,5)), aes(label = year), 
            family = "Proxima Nova T Cond", size = 8*pt2mm, fontface = "bold", color = neutral, alpha = .2) +
  geom_text_repel(data = data %>% filter(year > highlightedStartYear), aes(label = year), 
            family = "Proxima Nova T Cond", size = 8*pt2mm, fontface = "bold", color = dark) +
...
ggsave("plot.pdf", width = 5.5, height = 4, units = "in", useDingbats=FALSE)
